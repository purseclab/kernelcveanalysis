...
class ControlClassification(Enum):
    NONE = "none"
    DATA_ONLY = "data_only"
...
# JSON schema (informal) for LLM output. We keep it as a Python dict for prompt insertion
LLM_POSTCONDITION_SCHEMA = {
    "classification": ["none", "data_only", "limited_write", "ip_control", "arbitrary_krw", "arbitrary_rce"],
    "score": "int 0-100",
    "confidence": ["low", "medium", "high"],
    "ip_control": "bool",
    "arbitrary_write": "bool",
    "arbitrary_read": "bool",
    "write_offset_limited": "int|null",
    "funcptr_overwrite_target": "string|null",
    "evidence": "list of {id,file,line,text,reason}",
    "required_preconditions": "list of strings",
    "reproducible": "bool",
}

# Few-shot examples (compact) to show desired JSON structure and evidence citation style
LLM_FEW_SHOT_EXAMPLES = [
    {
        "desc": "Example 1: clear IP control via register fingerprint",
        "crash": "RIP: 0x41414141\nFault at address 0x41414141\nRAX: 0x41414141",
        "snippets": {
            "link:foo.c#123": {
                "file": "foo.c",
                "line": 123,
                "function_snippet": "    /* user buffer copied into kernel */\n    memcpy(kbuf, user_buf, len);\n    ret = kbuf[0];"
            }
        },
        "expected": {
            "classification": "ip_control",
            "score": 85,
            "confidence": "high",
            "ip_control": True,
            "evidence": [{"id": "link:foo.c#123", "file": "foo.c", "line": 123, "text": "memcpy(kbuf, user_buf, len)", "reason": "register RIP/RAX contains 0x41414141 fingerprint and user_buf present in snippet"}],
            "required_preconditions": ["user-controlled buffer copied into kernel via memcpy/copy_from_user"],
            "reproducible": False
        }
    },
    {
        "desc": "Example 2: bounded/limited write inside object",
        "crash": "Write of size 4 at addr 0xffff888012345678\nobject at 0xffff888012345600 cache kmalloc-64 of size 64 located 60 bytes inside of",
        "snippets": {
            "link:bar.c#200": {
                "file": "bar.c",
                "line": 200,
                "function_snippet": "    // index derived from user input\n    if (index >= arr_len) return -EINVAL;\n    arr[index] = value;"
            }
        },
        "expected": {
            ...
            "confidence": "medium",
            "ip_control": False,
            "write_offset_limited": 4,
            "evidence": [{"id": "link:bar.c#200", "file": "bar.c", "line": 200, "text": "arr[index] = value;", "reason": "write occurs 60 bytes into 64-byte object -> limited overwrite of up to 4 bytes"}],
            "required_preconditions": ["attacker controls index or supplies large index value"],
            "reproducible": False
        }
    },
    {
        "desc": "Example 3: info-leak via bounded read",
        "crash": "Read of size 8 at addr 0xffff8880deadbeef\nobject at 0xffff8880deadbe00 cache kmalloc-32 of size 32 located 24 bytes inside of",
        "snippets": {
            "link:baz.c#88": {
                "file": "baz.c",
                "line": 88,
                "function_snippet": "    // user-supplied length used\n    copy_to_user(uaddr, kernel_buf + offset, len);"
            }
        },
        "expected": {
            ...
            "ip_control": False,
            "arbitrary_read": True,
            "write_offset_limited": None,
            "evidence": [{"id": "link:baz.c#88", "file": "baz.c", "line": 88, "text": "copy_to_user(uaddr, kernel_buf + offset, len)", "reason": "copy_to_user indicates kernel memory copied to user -> potential info leak"}],
            "required_preconditions": ["attacker can trigger copy_to_user with crafted offset/len"],
            "reproducible": False
        }
    }
]
...
@dataclass
class PostCondition:
    classification: ControlClassification = ControlClassification.NONE
    score: int = 0  # 0-100
    confidence: ConfidenceLevel = ConfidenceLevel.LOW
    ip_control: bool = False
    reg_control: Dict[str, str] = field(default_factory=dict)
    arbitrary_write: bool = False
    arbitrary_read: bool = False
    write_offset_limited: Optional[int] = None
    funcptr_overwrite_target: Optional[str] = None
    memory_write_targets: List[Dict[str, Any]] = field(default_factory=list)
    evidence: List[str] = field(default_factory=list)
    required_preconditions: List[str] = field(default_factory=list)
    reproducible: bool = False
    sample_reproducer: Optional[str] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'reg_control': self.reg_control,
            'arbitrary_write': self.arbitrary_write,
            'arbitrary_read': self.arbitrary_read,
            'write_offset_limited': self.write_offset_limited,
            'funcptr_overwrite_target': self.funcptr_overwrite_target,
            'memory_write_targets': self.memory_write_targets,
            'evidence': self.evidence,
            'required_preconditions': self.required_preconditions,
            'reproducible': self.reproducible,
            'sample_reproducer': self.sample_reproducer,
        }



    def validate_llm_postcondition(llm_json: Optional[Dict[str, Any]], parsed: Dict[str, Any], snippets: Dict[str, Any], evidence: Dict[str, Any]) -> Dict[str, Any]:
        """Validate LLM-produced postcondition JSON against available parsed evidence.

        Returns a dict: {valid: bool, errors: [str], score: 0..1}
        """
        res = {"valid": False, "errors": [], "score": 0.0, "details": []}

        if not llm_json or not isinstance(llm_json, dict):
            res['errors'].append('no json')
            return res

        # Basic type and required field checks
        required = ['classification', 'score', 'confidence', 'ip_control', 'evidence']
        for k in required:
            if k not in llm_json:
                res['errors'].append(f'missing key {k}')

        # Type validation
        if 'score' in llm_json:
            try:
                sc = int(llm_json.get('score'))
                if sc < 0 or sc > 100:
                    res['errors'].append('score out of range 0-100')
            except Exception:
                res['errors'].append('score not integer')

        if 'confidence' in llm_json:
            if llm_json.get('confidence') not in ('low', 'medium', 'high'):
                res['errors'].append('confidence must be one of low/medium/high')

        # Validate evidence items more strictly
        evid = llm_json.get('evidence') or []
        evid_pass = 0
        evid_total = 0
        for e in evid:
            evid_total += 1
            if not isinstance(e, dict):
                res['errors'].append('evidence item not object')
                continue
            eid = e.get('id')
            file = e.get('file')
            line = e.get('line')
            ...
            matched = False
            # If id references snippet key directly
            if eid and eid in snippets:
                matched = True
                res['details'].append(f'evidence id {eid} matched snippet key')
            else:
                # try to match file+line to one of the provided snippets
                for k, s in (snippets or {}).items():
                    if not isinstance(s, dict):
                        continue
                    sf = s.get('file')
                    sl = s.get('line')
                    snippet_txt = s.get('function_snippet') or s.get('snippet') or ''
                    if file and sl and sf and file == sf and line and int(line) == int(sl):
                        matched = True
                        res['details'].append(f'evidence matched snippet by file+line: {file}:{line}')
                        break
                    # otherwise, if text is provided, check it is substring of any snippet
                    if text and text.strip() and text.strip() in snippet_txt:
                        matched = True
                        res['details'].append(f'evidence text matched in snippet {k}')
                        break
            ...
            if matched:
                ...
                if reason == 'insufficient_evidence':
                    res['details'].append('evidence declared insufficient for this item')
                else:
                    res['errors'].append(f'evidence item not matched: {reason or eid or text}')

        # Check arbitrary_write/arbitrary_read claims against static evidence counts
        if llm_json.get('arbitrary_write'):
            # require at least one snippet in evidence or evidence dict showing deref_writes or access_op==write
            write_ok = False
            for k, ev in (evidence or {}).items():
                if not isinstance(ev, dict):
                    continue
                try:
                    if int(ev.get('deref_writes', 0) or 0) > 0 or ev.get('access_op') == 'write':
                        write_ok = True
                        break
                except Exception:
                    continue
            if not write_ok:
                res['errors'].append('arbitrary_write asserted but no write evidence found in analyzed snippets')
        if llm_json.get('arbitrary_read'):
            read_ok = False
            for k, ev in (evidence or {}).items():
                if not isinstance(ev, dict):
                    continue
                try:
                    if int(ev.get('deref_reads', 0) or 0) > 0 or ev.get('access_op') == 'read':
                        read_ok = True
                        break
                except Exception:
                    continue
            if not read_ok:
                res['errors'].append('arbitrary_read asserted but no read evidence found in analyzed snippets')

        # Validate write_offset_limited against parsed object_info when available
        wol = llm_json.get('write_offset_limited')
        obj = parsed.get('object_info') or {}
        try:
            if wol is not None and obj:
                # wol is expected to be an integer <= remaining object bytes
                rem_ok = False
                if obj.get('obj_size') is not None and obj.get('offset') is not None:
                    try:
                        rem = int(obj.get('obj_size')) - int(obj.get('offset'))
                        if isinstance(wol, int) and wol <= rem:
                            rem_ok = True
                    except Exception:
                        rem_ok = False
                if not rem_ok:
                    res['errors'].append('write_offset_limited inconsistent with parsed object_info')
        except Exception:
            pass

        # Validate funcptr_overwrite_target: ensure the claimed function appears in frames or snippets
        fpt = llm_json.get('funcptr_overwrite_target')
        if fpt:
            found_target = False
            for f in (parsed.get('frames') or []):
                if f.get('func') and fpt in f.get('func'):
                    found_target = True
                    break
            if not found_target:
                # search snippets for the target string
                for k, s in (snippets or {}).items():
                    if isinstance(s, dict) and s.get('function_snippet') and fpt in s.get('function_snippet'):
                        found_target = True
                        break
            if not found_target:
                res['errors'].append('funcptr_overwrite_target not found in frames/snippets')

        # IP control cross-check (more robust): look for repeated-byte fingerprints or faulting IP/registers
        ip_claim = bool(llm_json.get('ip_control'))
        if ip_claim:
            raw = parsed.get('raw', '') or ''
            ip_ok = False
            # check for repeated 0x41 pattern or other 0xXX repeated patterns
            if re.search(r'0x([0-9a-fA-F]{2})\1{2,}', raw):
                ip_ok = True
            # check for RIP/EIP values with repeated bytes
            if re.search(r'\bRIP[:=]\s*0x[0-9a-fA-F]+', raw) or re.search(r'\bEIP[:=]\s*0x[0-9a-fA-F]+', raw):
                # if the RIP value matches any of the repeated-byte patterns, accept
                m = re.search(r'\bRIP[:=]\s*0x([0-9a-fA-F]+)', raw)
                if m:
                    val = m.group(1)
                    if re.match(r'^(41){2,}', val, re.I) or re.match(r'^(4141)', val, re.I):
                        ip_ok = True
            if not ip_ok:
                res['errors'].append('ip_control asserted but no fingerprint or matching RIP/EIP found in raw log')

        # Compose pass metrics
        # Weighted score: evidence match fraction (0.6) + write/read checks (0.2) + ip check (0.2 if ip claimed)
        evidence_score = (evid_pass / evid_total) if evid_total > 0 else 0.0
        signal_score = 0.0
        if llm_json.get('arbitrary_write'):
            signal_score += 0.2
        if llm_json.get('arbitrary_read'):
        ...
        agg = min(1.0, evidence_score * 0.6 + signal_score * 0.4)
        res['score'] = round(agg, 3)
        res['valid'] = res['score'] >= 0.8 and not res['errors']
        return res


    def merge_postconditions(heuristic_pc: Any, llm_json: Optional[Dict[str, Any]], validation: Dict[str, Any]) -> Dict[str, Any]:
        """Merge deterministic heuristic PostCondition (object or dict) with LLM JSON output.

        Rules:
          - If validation.score >= 0.75 and LLM confidence is high -> prefer LLM for main labels
          - Otherwise, prefer heuristic and attach LLM as alternate_view with provenance and validator info
        """
        if hasattr(heuristic_pc, 'to_dict'):
            base = heuristic_pc.to_dict()
        elif isinstance(heuristic_pc, dict):
            base = heuristic_pc
        else:
            base = {}

        merged = {'final': base.copy(), 'provenance': {'heuristic': True, 'llm': False}, 'llm': llm_json, 'validation': validation}

        if llm_json and validation.get('valid'):
            # interpret llm confidence
            conf = (llm_json.get('confidence') or '').lower() if isinstance(llm_json.get('confidence'), str) else ''
            high_conf = conf == 'high' or (llm_json.get('score') or 0) >= 70
            if validation.get('score', 0) >= 0.75 and high_conf:
                # accept LLM primary assertions
                merged['final'].update({k: llm_json.get(k) for k in ['classification', 'score', 'confidence', 'ip_control', 'arbitrary_write', 'arbitrary_read', 'write_offset_limited', 'funcptr_overwrite_target'] if k in llm_json})
                merged['provenance']['llm'] = True
                merged['final']['provenance'] = 'llm+heuristic'
            else:
                # keep heuristic but attach LLM as alternate
                merged['final']['llm_alternate'] = llm_json
                merged['final']['provenance'] = 'heuristic_preferred'
        else:
            merged['final']['provenance'] = 'heuristic_only'

        return merged


    def generate_trigger_c(repro_c: str, postcondition: Dict[str, Any]) -> str:
        """Generate a generic C harness intended to help trigger the given postcondition.

        This generator is heuristic and produces a template that an analyst can adapt.
        It supports common precondition patterns like: 'copy_from_user', 'ioctl', 'open file with crafted contents', and 'syz repro'.
        """
        header = """/* Auto-generated trigger harness (adapt device path / ioctl values as needed) */
    #include <stdio.h>
    #include <stdlib.h>
    #include <fcntl.h>
    #include <unistd.h>
    ...
    int main(int argc, char **argv) {
        const char *out_path = "/tmp/payload.bin";
        FILE *f = fopen(out_path, "wb");
        if (!f) { perror("fopen"); return 1; }
        // payload: repeated 0x41 pattern as common fingerprint
    ...
    """
        ...
        # heuristics from postcondition required_preconditions
        body = []
        reqs = postcondition.get('required_preconditions') or []
        reqs_text = " ".join(reqs).lower()
        if any(k in reqs_text for k in ('ioctl', 'device', '/dev')):
            # generate ioctl-based trigger template
            body.append("    // Open device and send payload via ioctl (fill DEVICE and IOCTL_CMD)\n")
            body.append("    int fd = open(\"/dev/vuln_device\", O_RDWR);\n")
            body.append("    if (fd < 0) { perror(\"open\"); return 1; }\n")
            body.append("    unsigned long IOCMD = 0xDEADBEEF; /* replace with actual ioctl code */\n")
            body.append("    void *buf = payload; size_t sz = sizeof(payload);\n")
            body.append("    int rc = ioctl(fd, IOCMD, buf);\n")
            body.append("    printf(\"ioctl rc=%d\n\", rc);\n")
            body.append("    close(fd);\n")
        elif any(k in reqs_text for k in ('openat', 'open', 'create file', 'filesystem', 'ext4', 'directory')):
            # write a file with crafted metadata (analyst must adapt for exact layout)
            body.append("    // Create a file with crafted contents that may trigger filesystem parsing bugs\n")
            body.append("    // Example: write payload as file contents; for metadata corruption you may need to craft a disk image and mount it\n")
            body.append("    printf(\"Creating test file /tmp/test_payload.bin\n\");\n")
            body.append("    // optionally invoke syscall to trigger the path, e.g., open/read/write\n")
            body.append("    int fd = open(\"/tmp/test_payload.bin\", O_CREAT|O_WRONLY, 0644);\n")
            body.append("    if (fd >= 0) { write(fd, payload, sizeof(payload)); close(fd); printf(\"wrote test file\n\"); }\n")
        elif 'syz' in (postcondition.get('sample_reproducer') or '').lower() or 'syzkaller' in reqs_text:
            body.append("    // Syzkaller-style repro: save payload and run syz executor with appropriate program (manual step)\n")
            body.append("    printf(\"Saved payload; run syz-executor with the repro and payload file.\n\");\n")
        else:
            # default: write payload and optionally perform a write() to a supplied fd (analyst supplies target)
            body.append("    // Default action: write payload to stdout (can be piped to target program or device)\n")
            body.append("    write(1, payload, sizeof(payload));\n")
...
def analyze_crash_to_postcondition(parsed: Dict[str, Any], snippets: Dict[str, Any], evidence: Dict[str, Any]) -> PostCondition:
    """Produce a PostCondition dataclass from parsed crash, snippets and evidence.

    This function implements the heuristics and scoring described in the design notes.
    It intentionally remains conservative when data is missing.
    """
    pc = PostCondition()
    score = 0
    ...
    # Reproducibility
    if 'syz' in raw or 'syzkaller' in raw or 'syz-executor' in raw:
        pc.reproducible = True
        score += 10

    # IP control detection
    # if any register equals a user-buffer pattern or obvious 0x414141.. pattern
    regs = {}
    # attempt to extract registers from raw (common log keys like RIP/EIP/PC)
    for m in re.finditer(r"\b(RIP|EIP|PC|RSP|RAX|RBX|RDI|RSI|RDX|RCX|R8|R9)[:=]\s*0x([0-9a-fA-F]+)", raw):
        regs[m.group(1)] = m.group(2)
    # naive user-controlled pattern check
    for rname, rval in regs.items():
        if re.match(r"^(41){2,}$", rval, re.I) or re.match(r"^414141", rval, re.I):
            pc.ip_control = True
            pc.evidence.append(f"register {rname} contains repeated 0x41 pattern -> user input fingerprint {rval}")
            score += 50
            break

    # If faulting IP appears in raw and points into user space (heuristic: address > 0xffff000000000000 on x86_64 is kernel, else user)
    m_ip = re.search(r"\bIP[:=]\s*0x([0-9a-fA-F]+)", raw)
    if m_ip:
        ipval = int(m_ip.group(1), 16)
        # conservative heuristic: if ip is non-canonical low pointer, treat as possible user
        if ipval < 0x00007fffffffffff:
            pc.ip_control = True
            pc.evidence.append(f"Faulting IP 0x{ipval:x} appears in user-space range")
            score += 40

    # Arbitrary write/read heuristics from evidence (memcpy/copy_from_user etc.)
    for k, ev in (evidence or {}).items():
        if not isinstance(ev, dict):
            continue
        if ev.get('access_op') == 'write' or ev.get('deref_writes', 0) > 0:
            pc.arbitrary_write = True
            pc.evidence.append(f"evidence of write in snippet {k}")
            score += 30
        if ev.get('access_op') == 'read' or ev.get('deref_reads', 0) > 0:
            pc.arbitrary_read = True
            pc.evidence.append(f"evidence of read/leak in snippet {k}")
            score += 15
        # detect copy_from_user as evidence of user-controlled input reaching kernel structures
        if any('copy_from_user' in ln for (_, ln) in ev.get('nearby_lines', []) if isinstance(ev.get('nearby_lines'), list)):
            pc.required_preconditions.append('user-controlled buffer copied into kernel via copy_from_user')
            score += 8

    # Boundedness and limited write detection
    if obj and obj.get('offset') is not None and obj.get('obj_size') is not None:
        off = int(obj.get('offset'))
        sz = int(obj.get('obj_size'))
        if off < sz and (sz - off) <= 32:
            pc.write_offset_limited = sz - off
            pc.evidence.append(f"write offset limited: offset {off} into size {sz}")
            pc.classification = ControlClassification.LIMITED_WRITE
            score += 12

    # funcptr overwrite detection heuristic: look for writes and frames in ops tables
    for f in (parsed.get('frames') or []):
        fn = f.get('func') or ''
        if re.search(r'file_ops|fops|netdev_ops|irq_handler|probe|->ops', fn, re.I) or re.search(r'file_operations|net_device_ops|ops', str(f.get('file') or ''), re.I):
            # moderate signal: write present in evidence
            if pc.arbitrary_write:
                pc.funcptr_overwrite_target = fn
                pc.evidence.append(f"stack frame {fn} suggests function-table or ops pointer target")
                score += 18
                pc.classification = ControlClassification.IP_CONTROL
                pc.ip_control = True

    # Final classification mapping based on flags
    if pc.ip_control:
        pc.classification = ControlClassification.IP_CONTROL
    elif pc.arbitrary_write and pc.arbitrary_read:
        pc.classification = ControlClassification.ARBITRARY_KRW
    elif pc.arbitrary_write:
        pc.classification = ControlClassification.LIMITED_WRITE if pc.write_offset_limited else ControlClassification.ARBITRARY_KRW
    elif pc.arbitrary_read:
        pc.classification = ControlClassification.DATA_ONLY

    # Normalize score to 0..100
    pc.score = max(0, min(100, int(score)))

    # Confidence buckets
    if pc.score >= 70:
        pc.confidence = ConfidenceLevel.HIGH
    elif pc.score >= 40:
        pc.confidence = ConfidenceLevel.MEDIUM
    else:
        pc.confidence = ConfidenceLevel.LOW

    return pc
"""
LLM adapter: allow using LiteLLM (OpenAI-compatible) or OpenAI SDK.

If you install `litellm` (pip install litellm), this code will route
chat completions through LiteLLM which supports OpenAI models using
the same `OPENAI_API_KEY`. If `litellm` isn't available, it falls back
to the official OpenAI Python SDK.
...
Optional:
- Set `LLM_BASE_URL` to point to an OpenAI-compatible endpoint
    (e.g., a proxy). If unset, defaults to OpenAI's API.
"""
def llm_chat(model: str, messages: list):
    """Unified chat completion that works with LiteLLM or OpenAI SDK.

    - Respects `OPENAI_API_KEY`.
    - Optional `LLM_BASE_URL` to override the default endpoint.
    - `messages` is an array of {role, content} dicts.
    Returns a dict with fields similar to OpenAI's chat completion.
    """
    api_key = os.getenv("OPENAI_API_KEY")
    base_url = os.getenv("LLM_BASE_URL")  # e.g., http://localhost:8000/v1 for proxies

    # if _LLM_BACKEND == "litellm":
        # LiteLLM auto-detects provider from model string; for OpenAI models,
        # it uses OPENAI_API_KEY. You can also set environment vars for other providers.
    ...
    return resp
    # else:
    #     # OpenAI SDK client; allow overriding base_url if a proxy is used
    ...
    #         **kwargs,
    #     )
    #     # Return SDK response as-is; callers can access `choices[0].message.content`
    #     return resp
    ...
    def _dedupe(lst):
        ...
        for e in lst:
            ...
            if key in seen:
                continue
            seen.add(key)
        # Basic validation of parsed_llm structure (lightweight)
        try:
            if parsed_llm and isinstance(parsed_llm, dict):
                ok = True
            else:
                ok = False
            out['parsed_valid_json'] = ok
        except Exception:
            out['parsed_valid_json'] = False
            out.append(e)
        return out
...
def analyze_ranges(snippet: str) -> Dict[str, Any]:
    ranges = {
        "array_indices": [],
        "pointer_offsets": [],
        "size_fields": []
    ...
    # Track array index expressions
    for m in re.finditer(r'\[([^\]]+)\]', snippet):
        idx = m.group(1).strip()
        if re.search(r'[0-9]', idx): # Has numeric component
            ranges["array_indices"].append(idx)
            
    # Track pointer arithmetic 
    for m in re.finditer(r'(\w+)\s*[+-]\s*([^;]+)', snippet):
        base = m.group(1)
        offset = m.group(2)
        ranges["pointer_offsets"].append(f"{base} +/- {offset}")
        
    # Look for size/length fields
    for m in re.finditer(r'(len|size|count|length)\s*=\s*([^;]+)', snippet, re.I):
        field = m.group(1)
        value = m.group(2)
        ranges["size_fields"].append(f"{field}={value}")
...
def stronger_heuristics(parsed: Dict[str, Any], snippets: Dict[str, Any], evidence: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Analyze post-conditions more thoroughly
    post_conditions = []

    # 1. Always capture KASAN state
    if "KASAN" in raw or "kasan" in raw:
        post_conditions.append("KASAN detected and logged invalid memory access")
        
    # 2. Capture specific crash impact
    if obj and obj.get("obj_size"):
        post_conditions.append(f"Memory corruption of object size {obj.get('obj_size')} bytes")
        
    # 3. Check for data leaks
    if "read" in prim.lower() or deref_reads > 0:
        leak_size = access.get("size", "unknown")
        post_conditions.append(f"Potential exposure of {leak_size} bytes of kernel memory")
        
    # 4. Check crash type impact
    if "null-ptr-deref" in kind:
        post_conditions.append("Kernel crash due to null pointer dereference")
    elif "use-after-free" in kind:
        post_conditions.append("Memory corruption of freed object space")

    report["postconditions"] = post_conditions

    # Check input range preconditions
    ranges = [] 
    if obj and obj.get("offset") is not None:
        ranges.append(f"Object offset must be >= {obj.get('offset')}")
    if obj and obj.get("obj_size"):
        ranges.append(f"Access size must be <= {obj.get('obj_size')} bytes")

    # Add numerical constraints
    for r in ranges:
        report["preconditions"].append(f"Input range constraint: {r}")
...
def get_openai_response(prompt: str, api_key: str, model: str = "gpt-5"):
    ...
    # Extract content across LiteLLM dict, LiteLLM ModelResponse, or OpenAI SDK object
    return resp.choices[0].message.content


# def openai_call_postcondition_function(evidence_pkg: Dict[str, Any], crash_excerpt: str, api_key: str, model: str = "gpt-5") -> Dict[str, Any]:
#     """
#     Call OpenAI with a function-calling schema that enforces the LLM to return structured
#     JSON matching LLM_POSTCONDITION_SCHEMA. Returns a dict with keys:
#       - ok: bool
#       - parsed: parsed JSON (or None)
#       - raw: raw LLM text
#       - error: optional error string
#     Falls back to returning ok=False when no valid structured output is obtained.
#     """
#     try:
#         client = OpenAI(api_key=api_key)
#         # build a JSON schema for the function
#         func_schema = {
#             "name": "return_postcondition",
#             "description": "Return a JSON object matching the postcondition schema (classification, score, confidence, ip_control, arbitrary_write, arbitrary_read, write_offset_limited, funcptr_overwrite_target, evidence, required_preconditions, reproducible).",
#             "parameters": {
#                 "type": "object",
#                 "properties": {
#                     "classification": {"type": "string", "enum": LLM_POSTCONDITION_SCHEMA.get('classification', [])},
#                     "score": {"type": "integer", "minimum": 0, "maximum": 100},
#                     "confidence": {"type": "string", "enum": ["low","medium","high"]},
#                     "ip_control": {"type": "boolean"},
#                     "arbitrary_write": {"type": "boolean"},
#                     "arbitrary_read": {"type": "boolean"},
#                     "write_offset_limited": {"oneOf": [{"type": "integer"}, {"type": "null"}]},
#                     "funcptr_overwrite_target": {"oneOf": [{"type": "string"}, {"type": "null"}]},
#                     "evidence": {
#                         "type": "array",
#                         "items": {"type": "object", "properties": {"id": {"type":"string"}, "file": {"type":"string"}, "line": {"type":"integer"}, "text": {"type":"string"}, "reason": {"type":"string"}}, "required": ["reason"]}
#                     },
#                     "required_preconditions": {"type": "array", "items": {"type": "string"}},
...
#         system_msg = {"role": "system", "content": "You are a precise assistant that must return a JSON object matching the provided function schema via a single function call."}
#         user_msg = {"role": "user", "content": "Please analyze the provided evidence and return the postcondition object via the function call. Evidence JSON follows."}

#         messages = [system_msg, user_msg, {"role": "user", "content": json.dumps({'evidence': evidence_pkg, 'crash_excerpt': crash_excerpt})}]

#         # resp = client.chat.completions.create(
#         model=model,
#         messages=messages,
#         try:
#             # Build an OpenAI tools/function schema for function calling
#             tool_schema = [{
#                 "type": "function",
#                 "function": {
#                     "name": "return_postcondition",
...
#             system_msg = {"role": "system", "content": "You are a precise assistant that must return a JSON object by calling the function 'return_postcondition'."}
#             user_msg = {"role": "user", "content": "Analyze the provided evidence and crash excerpt, then call return_postcondition with the JSON object."}

#             messages = [system_msg, user_msg, {"role": "user", "content": json.dumps({'evidence': evidence_pkg, 'crash_excerpt': crash_excerpt})}]

#             # Use llm_chat with tools and a specific tool_choice (function) name
#             resp = llm_chat(
#                 model=model,
#                 messages=messages,
#             )

#             # Extract tool/function call arguments across LiteLLM dict or OpenAI SDK object
#             args_text = None
...
#                 if tool_calls:
#                     args_text = tool_calls[0].get("function", {}).get("arguments", None)
#                 if not args_text:
...
#                     args_text = None
#                 if not args_text:
#                     args_text = msg.content
#             # Parse arguments JSON
#             parsed = None
#             if args_text:
#                 try:
#                     parsed = safe_json_loads(args_text)
#                 except Exception:
#                     parsed = None

#             if not parsed:
#                 return {"ok": False, "raw": args_text, "error": "failed to parse tool call JSON"}

#             return {"ok": True, "parsed": parsed, "raw": args_text}
#         except Exception as e:
...
"""
def llm_analyze_traces(crash: str, snippet_map: Dict[str, str], dynamic_results: Optional[Dict[str, Any]] = None, model_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    Inputs:
      ...
      - dynamic_results: optional dynamic analysis results from GDB tracing
    ...
    relevant source snippets, dynamic analysis findings, and asks the model to return:
      - a short precondition (concrete input ranges or states)
      - the reasoning/evidence lines
      - suggested reproduction steps
      - vulnerabilities detected through dynamic analysis

    If no LLM is available the function returns the assembled prompt under 'prompt' so
    it can be used manually.
    """
    prompt_lines = []
    prompt_lines.append("Task: Given the crash log, source snippets, and optional dynamic analysis results, produce a concise developer-friendly description of the PRECONDITION(s) required to reach the crash/vulnerability. Provide concrete input ranges and variable constraints. Analyze the full call chain from the crash point backwards and inspect all 'if/for/while/switch' conditions, early 'return' and 'goto' statements that restrict reaching the crash. For each such conditional, classify it as either an INPUT constraint (user-controlled) or KERNEL_STATE constraint. Also include short evidence lines from the provided snippets that justify each constraint.")
    prompt_lines.append("")
    ...
    if dynamic_results and dynamic_results.get('success'):
        prompt_lines.append("- \"dynamic_analysis\": runtime memory access traces, allocation/free events, and register states captured via GDB.")
        prompt_lines.append("")
        prompt_lines.append("NOTE: The dynamic analysis was performed WITHOUT KASAN enabled. This means bugs that would normally crash with KASAN may execute silently. Pay special attention to:")
        prompt_lines.append("  - Use-after-free patterns (accesses to freed memory addresses)")
        prompt_lines.append("  - Out-of-bounds accesses that don't trigger faults")
        prompt_lines.append("  - Memory corruption that may be detected through unexpected register values")
        prompt_lines.append("  - Allocation/deallocation patterns that suggest temporal safety violations")
    ...
    prompt_lines.append("    }")
    prompt_lines.append("  ],")
    prompt_lines.append("    \"postconditions\": [")
    prompt_lines.append("      {")
    prompt_lines.append("        \"state_after_crash\": \"<what kernel state is guaranteed after the crash>\",")
    prompt_lines.append("        \"kernel_impact\": \"<impact on kernel integrity/consistency>\",")
    prompt_lines.append("        \"error_states\": [\"<list of possible error states or outcomes>\"],")
    prompt_lines.append("        \"data_leaked\": \"<what data could be exposed if any>\",")
    prompt_lines.append("        \"controlability\": [\"<list of what can be controlled and how it can be controlled that is not in normal operations>\"]")
    prompt_lines.append("      }")
    prompt_lines.append("    ],")
    ...
    prompt_lines.append("")
    prompt_lines.append("=== CRASH LOG ===")
    prompt_lines.append(crash)
    
    prompt_lines.append("")
    prompt_lines.append("=== SOURCE SNIPPETS ===")
    count = 0
    ...
    # Add dynamic analysis results if available
    if dynamic_results and dynamic_results.get('success'):
        prompt_lines.append("")
        prompt_lines.append("=== DYNAMIC ANALYSIS RESULTS (GDB Tracing without KASAN) ===")
        prompt_lines.append(json.dumps({
            "memory_events": dynamic_results.get('events', []),
            "allocations_tracked": len(dynamic_results.get('allocations', {})),
            "frees_tracked": len(dynamic_results.get('frees', [])),
            "vulnerability_analysis": dynamic_results.get('vulnerability_analysis', {})
        }, indent=2))
        prompt_lines.append("")
        prompt_lines.append("IMPORTANT: Use the dynamic analysis to identify:")
        prompt_lines.append("1. Temporal bugs (UAF) - freed memory that is later accessed")
        prompt_lines.append("2. Spatial bugs (OOB) - accesses outside allocated regions")
        prompt_lines.append("3. Silent corruption - unexpected register values or memory patterns")
        prompt_lines.append("4. Race conditions - interleaved allocation/free patterns")
    ...
    # try:
    #     fc_res = openai_call_postcondition_function(evidence_pkg, crash[:2000], api_key)
    #     out = {"prompt": prompt, "evidence_pkg": evidence_pkg, "openai_raw": fc_res.get('raw'), "openai_parsed": fc_res.get('parsed'), "openai_meta": fc_res}
    #     # if function-calling returned a parsed dict, use it
    #     if fc_res.get('ok') and isinstance(fc_res.get('parsed'), dict):
    #         return out
    #     # otherwise fall through to a permissive fallback
    # except Exception:
    #     # non-fatal: continue to fallback
    #     out = {"prompt": prompt, "evidence_pkg": evidence_pkg}

    # # Fallback: simple instruction-based call (legacy behavior)
    # llm_instruction = (
    #     "Return a JSON object with keys: classification, score, confidence, ip_control (bool), "
    #     "arbitrary_write (bool), arbitrary_read (bool), write_offset_limited (int|null), funcptr_overwrite_target (str|null), "
    #     "evidence (list of {id,file,line,text,reason}), required_preconditions (list), reproducible (bool). "
    #     "Use only the provided evidence snippets. If unsure set fields to null/false and add an evidence item with reason 'insufficient_evidence'."
    # )

    # prompt_for_llm = llm_instruction + "\n\nEVIDENCE:=\n" + json.dumps(evidence_pkg) + "\n\nTASK:=\n" + crash

    # result = get_openai_response(prompt_for_llm, api_key)

    # # try to parse LLM JSON output
    # parsed_llm = safe_json_loads(result) if isinstance(result, str) else None

    # out.update({
    #     "openai_fallback_raw": result,
    #     "openai_fallback_parsed": parsed_llm,
    # })
...
def llm_generate_reproducer(llm_map: Dict[str, Any], reproducer: str) -> Dict[str, Any]:
    """
    Build a high-quality LLM prompt instructing GPT-5 to generate a *generic*
    c-style reproducer that satisfies all preconditions extracted from 
    the parsed vulnerability metadata, but allows variable postconditions.
    """
    parsed = llm_map.get("openai_llm").get("parsed")
    ...
    # Build human-readable sections for the LLM prompt
    precond_text = "\n".join(
        f"- {p['summary']}\n  Constraints:\n    " +
        "\n    ".join(p.get("concrete_constraints", [])) +
        f"\n  Why it hits: {p['why_reaches_crash']}"
        for p in preconditions
    )

    postcond_text = "\n".join(
        f"- {p['state_after_crash']}\n  Impact: {p['kernel_impact']}\n  Errors: " +
        ", ".join(p.get("error_states", []))
        for p in postconditions
    )

    evidence_text = "\n".join(
        f"- {e['file']}:{e['line']} — {e['note']}"
        for e in evidence
    )
    ...
    prompt_lines = [
        ...
        "=== TASK ===",
        "Given:",
        "1. A detailed vulnerability analysis including exploitability, required",
        "   preconditions, and evidence.",
        "2. The full syzkaller reproducer C program.",
        "",
        "Your job is to generate a *generic c reproducer template* that:",
        "  • ALWAYS satisfies all required preconditions",
        "  • Abstracts away the exact numeric values needed for triggering the bug",
        "  • Allows the USER to selectively choose *which* postcondition scenario",
        "    they want to achieve (e.g., KASAN OOB read, GPF, silent corruption)",
        "  • Contains clearly marked placeholders for user-controlled knobs",
        "  • Preserves the logical structure required to reproduce the vulnerability",
        "  • Removes unnecessary syzkaller boilerplate, keeping only core logic",
        "",
        "The template should:",
        "  - Allow selecting segment numbers, sizes, fallocate offsets, etc.",
        "  - Allow users to tune memory layout or allocation patterns",
        "  - Preserve the necessary filesystem crafting steps",
        "  - Never violate the preconditions",
        "",
        "Do NOT produce any non-code text; produce only c code.",
        "",
        "=== VULNERABILITY OVERVIEW ===",
        f"Exploitability: {overview.get('exploitability', '')}",
        f"Rationale: {overview.get('rationale', '')}",
        f"Primitive Capabilities: {overview.get('primitive_capabilities', '')}",
        "",
        "=== REQUIRED PRECONDITIONS (MUST ALWAYS BE SATISFIED) ===",
        precond_text,
        "",
        "=== POSSIBLE POSTCONDITIONS (USER-SELECTABLE OUTCOMES) ===",
        postcond_text,
        "",
        "=== TRACE & EVIDENCE LOCATIONS ===",
        evidence_text,
        "",
        "=== ORIGINAL SYZKALLER REPRODUCER (CONTEXT ONLY) ===",
        "```c",
        reproducer,
        "```",
        "",
        ...
        "",
        "GENERIC REPRODUCER TEMPLATE (c code only, no commentary)",
        "",
        "Now generate the generic reproducer."
    ]

    # Assemble final prompt
    prompt = "\n".join(prompt_lines)    # Run local LLM
    # llm_out = _call_llm(prompt, model_path=model_path)
    # Run OpenAI LLM
    api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get("OPENAI_KEY")
    if not api_key:
    ...
    repro = get_openai_response(prompt, api_key)

    return repro


def generate_llm_trigger_c(original_repro_c: str, postcondition: Dict[str, Any], 
                           static_analysis: Dict[str, Any], dynamic_analysis: Optional[Dict[str, Any]],
                           crash_log: str) -> str:
    """
    Use LLM to generate an enhanced C trigger based on comprehensive analysis.
    
    This combines static analysis (postconditions, evidence) with optional dynamic
    analysis (memory accesses, register states) to produce a more targeted reproducer.
    """
    
    # Build comprehensive context for LLM
    prompt_lines = [
        "You are an expert kernel security researcher and exploit developer.",
        "",
        "=== TASK ===",
        "Generate an optimized C program to trigger the kernel vulnerability described below.",
        "The reproducer should be:",
        "  • Minimal and focused on the core vulnerability",
        "  • Well-commented explaining each step",
        "  • Configurable with #define macros for key parameters",
        "  • Executable with minimal dependencies (libc only)",
        "",
        "=== VULNERABILITY ANALYSIS ===",
        "",
    ]
    
    # Add postcondition info
    if postcondition:
        prompt_lines.append(f"**Classification:** {postcondition.get('classification', 'unknown')}")
        prompt_lines.append(f"**Confidence:** {postcondition.get('confidence', 'unknown')}")
        prompt_lines.append(f"**Exploitability Score:** {postcondition.get('score', 0)}/100")
        prompt_lines.append(f"**IP Control:** {'YES' if postcondition.get('ip_control') else 'NO'}")
        prompt_lines.append(f"**Arbitrary Read:** {'YES' if postcondition.get('arbitrary_read') else 'NO'}")
        prompt_lines.append(f"**Arbitrary Write:** {'YES' if postcondition.get('arbitrary_write') else 'NO'}")
        
        if postcondition.get('write_offset_limited'):
            prompt_lines.append(f"**Write Offset Limited:** {postcondition['write_offset_limited']} bytes")
        
        if postcondition.get('required_preconditions'):
            prompt_lines.append("")
            prompt_lines.append("**Required Preconditions:**")
            for pc in postcondition['required_preconditions']:
                prompt_lines.append(f"  - {pc}")
        
        prompt_lines.append("")
    
    # Add dynamic analysis findings if available
    if dynamic_analysis and dynamic_analysis.get('success'):
        prompt_lines.append("=== DYNAMIC ANALYSIS FINDINGS ===")
        prompt_lines.append("(Runtime behavior captured without KASAN)")
        prompt_lines.append("")
        
        vuln_analysis = dynamic_analysis.get('vulnerability_analysis', {})
        if vuln_analysis.get('uaf_detected'):
            prompt_lines.append("⚠️  **USE-AFTER-FREE DETECTED**")
            for vuln in vuln_analysis.get('vulnerabilities', []):
                if vuln.get('type') == 'use-after-free':
                    prompt_lines.append(f"  - Address: {vuln.get('address')}")
                    prompt_lines.append(f"  - Size: {vuln.get('size')} bytes")
        
        if vuln_analysis.get('oob_detected'):
            prompt_lines.append("⚠️  **OUT-OF-BOUNDS ACCESS DETECTED**")
        
        if dynamic_analysis.get('events'):
            prompt_lines.append(f"  - Memory events captured: {len(dynamic_analysis['events'])}")
        if dynamic_analysis.get('allocations'):
            prompt_lines.append(f"  - Allocations tracked: {len(dynamic_analysis['allocations'])}")
        
        prompt_lines.append("")
    
    # Add crash log excerpt
    prompt_lines.append("=== CRASH LOG (excerpt) ===")
    crash_lines = crash_log.split('\n')[:30]  # First 30 lines
    prompt_lines.extend(crash_lines)
    prompt_lines.append("...")
    prompt_lines.append("")
    
    # Add original reproducer if available
    if original_repro_c:
        prompt_lines.append("=== ORIGINAL REPRODUCER (for reference) ===")
        prompt_lines.append("```c")
        # Truncate if too long
        repro_lines = original_repro_c.split('\n')
        if len(repro_lines) > 100:
            prompt_lines.extend(repro_lines[:50])
            prompt_lines.append("... (truncated) ...")
            prompt_lines.extend(repro_lines[-20:])
        else:
            prompt_lines.append(original_repro_c)
        prompt_lines.append("```")
        prompt_lines.append("")
    ...
    prompt_lines.extend([
        ...
        "1. Includes necessary headers and defines",
        "2. Has configurable parameters as #define macros at the top",
        "3. Implements the minimum steps to trigger the vulnerability",
        "4. Includes comments explaining the vulnerability trigger mechanism",
        "5. Handles errors gracefully with perror() and returns appropriate exit codes",
        "6. If UAF is detected, includes allocation/free sequence with timing",
        "7. If specific memory addresses or values are important, uses macros",
        "",
        "Output ONLY valid C code, no explanatory text before or after.",
        ""
    ])
    ...
    if not api_key:
        # Fallback to heuristic generator if no API key
        print("[!] No OpenAI API key found, using heuristic generator")
        return PostCondition.generate_trigger_c(original_repro_c or "", postcondition)
    
    try:
        enhanced_trigger = get_openai_response(prompt, api_key)
        # Clean up markdown code blocks if present
        enhanced_trigger = re.sub(r'^```c?\n', '', enhanced_trigger)
        enhanced_trigger = re.sub(r'\n```$', '', enhanced_trigger)
        return enhanced_trigger
    except Exception as e:
        print(f"[!] LLM trigger generation failed: {e}, using fallback")
        return PostCondition.generate_trigger_c(original_repro_c or "", postcondition)


def create_reproducer(llm_analysis: Dict[str, Any], original_repro_c: str, out_dir: str) -> Dict[str, Any]:
    """
    Generate a generic C reproducer via LLM, save it, compile it, and if
    compilation fails, feed the error back to the LLM to fix and recompile.

    Returns a dict with keys:
      - source_path: path to the generated C source
      - binary_path: path to the compiled binary (if compiled)
      - compiled: bool indicating if compilation succeeded
      - error: compiler stderr output if compilation failed
      - source: the generated C source code (final version)
    """
    os.makedirs(out_dir, exist_ok=True)

    # Step 1: generate generic reproducer code
    try:
        generated = llm_generate_reproducer(llm_analysis, original_repro_c)
    except Exception as e:
        return {"compiled": False, "error": f"llm_generate_reproducer failed: {e}"}

    source_path = os.path.join(out_dir, "repro_generic.c")
    with open(source_path, "w") as f:
        f.write(generated)

    binary_path = os.path.join(out_dir, "repro_generic")
    cmd = ['./compile_x86_64.sh', source_path, binary_path]
    proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    success = proc.returncode == 0 and os.path.isfile(binary_path)
    err = proc.stderr

    if success:
        return {"source_path": source_path, "binary_path": binary_path, "compiled": True, "error": "", "source": generated}

    # Step 2: if compilation failed, attempt LLM fix using compiler error
    api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get("OPENAI_KEY")
    ...
    try:
        ...
        return {"source_path": source_path, "binary_path": binary_path, "compiled": False, "error": f"fix attempt failed: {fe}\nOriginal error: {err}", "source": src}
...
def analyze(crash_text: str, repro_path: str, source_root: Optional[str] = None, dynamic_results: Optional[dict] = None) -> Dict[str, Any]:
    ...
    for k,v in evidence.items():
        if isinstance(v, dict):
            ranges[k] = analyze_ranges(v.get("snippet", ""))
            
    # result["input_ranges"] = ranges

    # Add input influence analysis
    influences = analyze_input_influence(parsed, snippets)
    # result["input_influences"] = influences
    ...
    try:
        ...
        # Guard against None or empty repro_path before filesystem ops
        if repro_path and repro_path != "" and os.path.isfile(repro_path):
            with open(repro_path, "r") as rf:
                repro_content = rf.read()
            out_dir = os.path.dirname(repro_path)
...
def analyze_input_influence(parsed: Dict[str, Any], snippets: Dict[str, Any]) -> Dict[str, Any]:
    influences = {
        "syscall_args": [],
        "user_copies": [], 
        "controlled_fields": [],
    ...
    # Track user->kernel copies
    for k,v in snippets.items():
        if isinstance(v, dict):
            txt = v.get("snippet", "")
            # Find copy_from_user
            for m in re.finditer(r'copy_from_user\((.*?)\)', txt):
                influences["user_copies"].append(m.group(1))
                
            # Find controlled fields
            for m in re.finditer(r'->([a-z_]+)\s*=\s*([^;]+)', txt):
                field = m.group(1) 
                value = m.group(2)
                if re.search(r'user|from_user|copy', value, re.I):
                    influences["controlled_fields"].append(field)
