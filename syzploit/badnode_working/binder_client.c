...
int binder_acquire_done(int fd, binder_uintptr_t node_id, binder_uintptr_t cookie) {
    uint32_t write_buf[5];
    write_buf[0] = BC_ACQUIRE_DONE;
    memcpy(&write_buf[1], &node_id, sizeof(node_id));
    memcpy(&write_buf[3], &cookie, sizeof(cookie));
    if (binder_write(fd, write_buf, sizeof(write_buf)) < 0) {
        return -1;
    }
    return 0;
...
int binder_increfs_done(int fd, binder_uintptr_t node_id, binder_uintptr_t cookie) {
    uint32_t write_buf[5];
    write_buf[0] = BC_INCREFS_DONE;
    memcpy(&write_buf[1], &node_id, sizeof(node_id));
    memcpy(&write_buf[3], &cookie, sizeof(cookie));
    if (binder_write(fd, write_buf, sizeof(write_buf)) < 0) {
        return -1;
