...
def main():
    parser = argparse.ArgumentParser(
        description="Raw UDP Sniffer and ICMP Frag Needed Responder."
    )
    parser.add_argument(
        "-i",
        "--interface",
        required=True,
        help="Network interface to bind to (e.g., br0testvm).",
    )
    parser.add_argument(
        "-m",
        "--mtu",
        type=int,
        default=1200,
        help="Next-Hop MTU to advertise (default: 1200).",
    )
    parser.add_argument(
        "-p",
        "--port",
        type=int,
        help="Optional: UDP port to bind and silently absorb traffic (prevents Port Unreachable errors).",
    )
    ...
    try:
        ...
        raw_recv_sock.setsockopt(
            socket.SOL_SOCKET, SO_BINDTODEVICE, args.interface.encode("utf-8")
        )
    ...
    if args.port:
        try:
            ...
            logging.info(
                f"Bound dummy UDP socket on port {args.port} to suppress 'Port Unreachable' messages."
            )
        except Exception as e:
        ...
        while True:
            ...
            for sock in readable:
                if sock is dummy_sock:
                    ...
                    logging.info(
                        f"Intercepted UDP packet from {sender_ip}. Crafting ICMP reply..."
                    )
