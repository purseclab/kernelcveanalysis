...
#define RETURN_ON_FALSE(x) ({if (!(x)) return false;})
    ...
    // MODIFIED: print out kmap_size
    LOGD("ctx->kmap_size = %u\n", ctx->kmap_size);
    ctx->kmap = SYSCHK(mmap(NULL, ctx->kmap_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0));

    // MODIFIED: look for _head
    const unsigned char head[] = { 0x4d, 0x5a, 0x00, 0x91, 0xff, 0xbf, 0xa5, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xec, 0x02, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x41, 0x52, 0x4d, 0x64, 0x40, 0x00, 0x00, 0x00 };
    size_t head_offset = memmem(ctx->kmap, ctx->kmap_size, head, sizeof(head));
    LOGD("head offset: %p\n", head_offset);
