...
## 2026-08-19 — Robust fixing-commit dataset regeneration

- Goal: encode the prior missing-patch research in Python so `extract_fix_commits.py` can regenerate the enriched dataset without manual intervention.
- Pre-edit checkpoint: `checkpoints/2026-08-19_before_robust_commit_providers/`.
- Parser expansion: added legacy kernel.org cgit and `/linus/` forms, arbitrary GitHub and GitLab commits when NVD labels the reference as a patch, and Android Gitiles links with base64 patch decoding plus abbreviated-hash resolution.
- Verified providers: added CVE-scoped Android Security Bulletin row parsing, kernelCTF `metadata.json` parsing, explicitly mapped and revalidated Linux-CVE alias announcements, Dirty Pipe's researcher advisory, and Ubuntu kernel-team cherry-pick metadata. Each output record now identifies its provider, discovery URL, and commit scope.
- Conservative matching: generic project commits require NVD's `Patch` tag; Android parsing is restricted to the requested CVE's table row; Linux-CVE aliases must retain explicit fix/address wording; Dirty Pipe accepts only the link whose anchor states it was fixed; Ubuntu mail must name the requested CVE and label a hash as `cherry picked from commit`.
- Caching/reproducibility: NVD responses, advisory/metadata pages, Gitiles commit metadata, and patch bodies are cached separately. Atomic writes are retained. An immediate cached strict rerun completed in 1 second and reproduced the exact SHA-256 `4655989e501b47c4d2ea70e56725db3868a4c3ff338dea894793864f4b347dcd`.
- Result: 503 unique finding/commit records with 503 non-empty patches and zero errors: 459 NVD references, 22 Android bulletin commits, 11 Linux-CVE alias commits, 8 kernelCTF commits, 1 Ubuntu mailing-list commit, 1 researcher-advisory commit, and 1 generated non-CVE finding. This covers 140 of 165 CVEs; 25 remain without a defensible commit ID.
- Multiple fixes: output remains one record per CVE/commit association, so stable backports and multi-part fixes are all retained and independently carry patch text. Duplicate URL forms resolving to the same CVE/commit pair are collapsed.
- Validation: 11 unit tests passed; Python compilation passed; strict full generation and strict cached regeneration passed; record/summary counts agree; all patch bodies are non-empty; and no duplicate finding/commit association exists.
- Final checkpoint: `checkpoints/2026-08-19_robust_commit_providers_generated/`.

## 2026-08-19 — Normalized commit-tree category

- Goal: make each fixing-commit record directly distinguish Linux stable, Linux mainline/upstream, Android kernel, and other project trees.
- Pre-edit checkpoint: `checkpoints/2026-08-19_before_commit_tree_field/`.
- Schema change: bumped the output to schema version 3 and added `commit_tree` to every record. The exhaustive mapping is `stable-linux` → `stable`, `upstream-linux` → `mainline`, `android-kernel` → `android`, and all remaining scopes → `other`; the detailed `commit_scope` remains available.
- Summary change: added `summary.by_commit_tree` and documented the normalized field in `methodology.commit_tree`.
- Regeneration command: `python3 extract_fix_commits.py --strict`, using the populated NVD/source/patch caches.
- Result: all 503 records contain a valid category: 407 `stable`, 66 `mainline`, 22 `android`, and 8 `other`; all 503 patches remain present and the run reported zero errors.
- Validation: 12 unit tests passed; Python compilation passed; schema/count assertions passed; no record is missing the field and no value falls outside the four-value vocabulary.
- Final checkpoint: `checkpoints/2026-08-19_commit_tree_field_generated/`.

## 2026-08-21 — Audit of the 25 remaining CVEs

- Goal: determine whether each CVE in `cves_without_linux_commit_urls` truly lacks a fix commit or is missing because its evidence takes a form unsupported by the extractor.
- Pre-audit checkpoint: `checkpoints/2026-08-21_before_missing_fix_audit/`.
- Method: inspected the cached NVD records, each finding's original source, vendor/upstream trackers, mailing-list patches, pull requests, release/tag histories, alias status, and upstream repositories. Direct patch endpoints were fetched for representative recovered hashes to verify that they resolve to complete mail-style patches.
- Result: 19 CVEs have specific commit attribution; 3 more have strong release/tag-history candidates; CVE-2023-2640 has a complete Ubuntu-specific patch series but no single portable hash; CVE-2024-3094 has release/rollback remediation rather than a conventional fix commit; and CVE-2025-0927 was rejected by the Linux CNA.
- Principal root causes: hashes embedded in description prose, source URLs not crawled, patch-email-to-merge resolution, pull-request expansion, SVN identifiers, retired forges, fixed-version/tag inference, sourceware metadata, Ubuntu release/flavour-specific trees, and rejected/duplicate aliases.
- Artifact: `MISSING_FIX_COMMITS_AUDIT.md` records the per-CVE result, commit candidates, provenance qualification, and parser gap.

## 2026-08-30 — Driver-focused LPE research sweep

- Goal: expand the dataset with source-backed Linux LPEs centered on kernel drivers and kernel components, including Android upstream-kernel EoP issues and carefully selected out-of-tree Linux drivers/filesystems; add other qualifying local-escalation findings found during the same sweep.
- Pre-edit checkpoints: `checkpoints/2026-08-30_before_driver_lpe_research/` and `checkpoints/2026-08-30_before_commit_dedup_fix/`.
- Sources checked: targeted Ubuntu CVE pages; Openwall raw MIDI, Intel i915, Linux-CVE, and current security disclosures; ZDI's tcindex advisory; official Android Security Bulletins for February, September, and December 2025; Android OSV; NVIDIA Linux GPU Display Driver bulletins; the Linux kernel CNA CVE record; Qualys CrackArmor research; OpenZFS/GitHub Advisory and Ubuntu; NVD and vendor corroboration.
- Added 15 CVE records to `MANUAL_RECORDS`: CVE-2018-10902 (raw MIDI), CVE-2022-0330 (i915), CVE-2023-1829 (tcindex), CVE-2023-3090 (ipvlan), CVE-2023-39198 (QXL), CVE-2024-53104 (UVC), CVE-2025-23244/23282/33219 (NVIDIA out-of-tree display driver), CVE-2025-38352 (POSIX CPU timer), CVE-2025-48623 (pKVM), CVE-2025-48624 (arm-smmu-v3 IOMMU), CVE-2025-71089 (IOMMU SVA), CVE-2026-23268 (AppArmor), and CVE-2026-79619 (OpenZFS out-of-tree `/dev/zfs` ioctl path).
- Existing CVE-2025-38500 was not duplicated: it already has a stronger high-certainty kernelCTF record; the December 2025 Android bulletin and Android OSV evidence were added to the source audit as corroboration.
- Negative research: CVE-2025-21755 was excluded because NVD and Red Hat record the upstream CVE as rejected despite an Android bulletin row. CVE-2025-38474 was excluded because the checked Sierra USB-driver evidence supports denial of service but did not provide a defensible direct LPE/RCE claim. Other Android rows that were only DoS, information disclosure, or insufficiently scoped were not promoted.
- Certainty decisions: tcindex and CVE-2025-38352 are high because the checked sources provide direct code-execution/active-exploitation evidence. Vendor-only, official-bulletin-only, and potential-impact cases remain medium; prerequisites and lack of a verified public exploit are recorded in `caveat`. No exploit source was copied or executed.
- Base regeneration: `python3 generate_dataset.py` produced 180 CVEs (175 LPE, 5 RCE), with 134 high-certainty and 46 medium-certainty records. The non-CVE artifact remains one finding. The baseline comparison found 15 additions, no removals, and no changes to the 165 prior records.
- Fixing-commit regeneration: `python3 extract_fix_commits.py --strict` initially found one stale duplicate URL failure for an existing CVE-2025-38500 kernelCTF/NVD association. The extractor now deduplicates commit links by finding/commit ID before patch retrieval, preserving one association and preventing a bad duplicate endpoint from masking a valid patch. The focused regression test was added to `test_extract_fix_commits.py`.
- Final cached strict regeneration completed with 543 fixing-commit records, 543 complete patches, and zero errors: 542 CVE associations plus one non-CVE association; 31 CVEs remain without a defensible fixing commit from implemented providers. Provider totals are 499 NVD-reference, 22 Android-bulletin, 11 Linux-CVE-alias, 8 kernelCTF-metadata, 1 Ubuntu-mailing-list, 1 researcher-advisory, and 1 non-CVE record. Tree totals are 441 stable, 69 mainline, 25 Android, and 8 other.
- Validation: `python3 -m py_compile generate_dataset.py extract_fix_commits.py test_extract_fix_commits.py` passed; `python3 -m unittest -v test_extract_fix_commits.py` passed all 13 tests; JSON/schema, uniqueness, required-field, patch-text, summary, and allowed `commit_tree` checks passed. The unauthenticated NVD run used the documented six-second pacing and completed successfully.
- Final checkpoint: `checkpoints/2026-08-30_driver_lpe_research_generated/`.

## 2026-08-31 — Broad Linux and Android LPE/RCE expansion

- Goal: continue expanding the dataset beyond driver-focused findings, covering any Linux or qualifying Android LPE/RCE with direct source-backed evidence.
- Pre-edit checkpoint: `checkpoints/2026-08-30_broad_lpe_research_before_edits/`.
- Sources checked: Exodus Intelligence and the Linux kernel CNA for the nftables CVE-2026-23111; the Linux kernel CNA plus STAR Labs reporting for the act_api CVE-2026-53264; Red Hat and Ubuntu CIFS trackers for CVE-2026-46243; the udisks2 upstream advisory with Red Hat/Debian corroboration for CVE-2026-7867; util-linux maintainer advisories and Ubuntu patch pages for CVE-2026-53613 and CVE-2026-53614; the libinput Openwall disclosure with NVD/Red Hat corroboration for CVE-2026-50292; and official Android March, June, and Pixel June 2026 bulletins for qualifying upstream-kernel/kernel-component EoP rows.
- Added 23 CVE records: high-certainty Linux LPEs CVE-2026-23111, CVE-2026-53264, CVE-2026-53613, CVE-2026-53614, and CVE-2026-50292; medium-certainty CIFS and udisks2 LPEs CVE-2026-46243 and CVE-2026-7867; 13 upstream-kernel/kernel-component Android March rows (CVE-2024-43859, CVE-2025-38618, CVE-2025-39682, CVE-2025-39946, CVE-2025-40266, CVE-2026-0027, CVE-2026-0028, CVE-2026-0029, CVE-2026-0030, CVE-2026-0031, CVE-2026-0032, CVE-2026-0037, and CVE-2026-0038); Android June CVE-2025-40214; and Pixel kernel-component CVE-2026-0125 and EdgeTPU Kernel Driver CVE-2026-0137.
- Android scope decision: only official-bulletin rows explicitly identified as upstream-kernel or kernel-component EoP were promoted. Android System-only rows, July's no-vulnerability bulletin, August rows without a qualifying exposed kernel entry, DoS/ID rows, and proprietary Pixel components without qualifying scope were not added. The official Android kernel-components table's System-execution prerequisite is preserved in the relevant caveats.
- Certainty decisions: the nftables, act_api, util-linux, and libinput records are high because the checked public research or maintainer disclosures report root escalation/code execution; the CIFS, udisks2, and official Android/Pixel bulletin records are medium because direct EoP classification was verified without a detailed public exploit demonstration. No exploit code was copied, compiled, executed, or debugged.
- Extractor change: added a narrow source-backed CVE advisory provider for the two util-linux CVEs. It validates the CVE page, the patch-details section, and each expected full upstream commit URL before fetching patches; a regression test covers successful extraction and incomplete-page rejection. This handles the temporary absence of NVD records for these newly assigned CVEs without fabricating NVD metadata.
- Base regeneration: `python3 generate_dataset.py` produced 203 unique CVEs (198 LPE, 5 RCE), with 139 high-certainty and 64 medium-certainty records. The non-CVE artifact remains one finding. Comparison with the prior generated checkpoint found 23 additions, no removals, and no changes to the 180 previous records.
- Fixing-commit regeneration: the initial restricted-network run failed NVD resolution for 23 new CVEs; after network access was restored, `python3 extract_fix_commits.py --strict` completed with 627 records, 627 non-empty patches, and zero errors. This adds 84 fixing associations while preserving all 543 previous associations unchanged. The derived output has 626 CVE associations plus one non-CVE association, 34 CVEs without a defensible commit from implemented providers, 50 Android-tree records, 69 mainline records, 13 other-project records, and 495 stable-tree records. The three newly added CVEs without a discovered fix association are CVE-2026-0125, CVE-2026-0137, and CVE-2026-7867.
- Validation: `python3 -m py_compile generate_dataset.py extract_fix_commits.py test_extract_fix_commits.py` passed; `python3 -m unittest -v test_extract_fix_commits.py` passed all 14 tests; JSON/schema, uniqueness, allowed vocabulary, required commit fields, non-empty patch, summary, and baseline-association checks passed.
- Final checkpoint: `checkpoints/2026-08-31_broad_lpe_research_generated/`.
