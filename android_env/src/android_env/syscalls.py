# mapping from syscall names to numbers
# sourced from https://github.com/david942j/seccomp-tools/blob/master/lib/seccomp-tools/consts/sys_nr/aarch64.rb
SYSCALLS_AARCH64 = {
    'io_setup': 0,
    'io_destroy': 1,
    'io_submit': 2,
...
# from https://github.com/torvalds/linux/blob/449c2b302c8e200558619821ced46cc13cdb9aa6/scripts/syscall.tbl#L4
LINUX_ARM64_SYSCALL_TBL = '''
...
def parse_syscall_tbl(tbl: str) -> dict[str, int]:
    out = {}

    # see https://www.chromium.org/chromium-os/developer-library/reference/linux-constants/syscalls/#arm64-64-bit
    # for seeing which ones were used in practice
    used_subsystems = {
        'common',
        '64',
        'rlimit',
    ...
    for line in tbl.split('\n'):
        ...
        if subsystem in used_subsystems:
            assert name not in out
            out[name] = number
    
    return out

def get_syscall_map() -> dict[str, int]:
    return parse_syscall_tbl(LINUX_ARM64_SYSCALL_TBL)