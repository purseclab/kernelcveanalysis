...
void execute_one(void)
{
	...
	NONFAILING(*(uint16_t*)0x20000048 = 0x28de);
	NONFAILING(*(uint16_t*)0x2000004a = 0x1102);
	...
	NONFAILING(*(uint16_t*)0x2000006b = 7);
	...
	res = -1;
	NONFAILING(res = syz_usb_connect(/*speed=*/0, /*dev_len=*/0x36, /*dev=*/0x20000040, /*conn_descs=*/0));
	if (res != -1)
		r[0] = res;
	syscall(__NR_bpf, /*cmd=*/5ul, /*arg=*/0ul, /*size=*/0ul);
	NONFAILING(syz_usb_control_io(/*fd=*/r[0], /*descs=*/0, /*resps=*/0));
	NONFAILING(*(uint32_t*)0x200002c0 = 0x24);
	NONFAILING(*(uint64_t*)0x200002c4 = 0);
	NONFAILING(*(uint64_t*)0x200002cc = 0);
	NONFAILING(*(uint64_t*)0x200002d4 = 0x20000000);
	NONFAILING(*(uint8_t*)0x20000000 = 0);
	NONFAILING(*(uint8_t*)0x20000001 = 0x22);
	NONFAILING(*(uint32_t*)0x20000002 = 7);
	NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000006, 3, 0, 2));
	NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000006, 0, 2, 2));
	NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000006, 0, 4, 4));
	NONFAILING(memcpy((void*)0x20000007, "\x6e\x35\x65\xc0", 4));
	NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x2000000b, 1, 0, 2));
	NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x2000000b, 0, 2, 2));
	NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x2000000b, 0xb, 4, 4));
	NONFAILING(memset((void*)0x2000000c, 105, 1));
	NONFAILING(*(uint64_t*)0x200002dc = 0);
	NONFAILING(syz_usb_control_io(/*fd=*/r[0], /*descs=*/0x200002c0, /*resps=*/0));
	NONFAILING(memcpy((void*)0x20000080, "/dev/hidraw#\000", 13));
	NONFAILING(syz_open_dev(/*dev=*/0x20000080, /*id=*/0, /*flags=*/0));
