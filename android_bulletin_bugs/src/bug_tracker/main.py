...
class Bug:
    ...
    def is_after_security_bulletin(self, year: int, month: int) -> bool:
        bug_year, bug_month = self.security_bulletin.split("-")
        return (13 * year + month) < (13 * int(bug_year) + int(bug_month))
...
class Db:
    ...
    def add_bugs(self, bugs: list[Bug]):
        ...
        sql_input = [
            (
                ...
                bug.component,
                None
                if bug.updated_versions is None
                else ",".join(bug.updated_versions),
                bug.security_bulletin,
            )
            for bug in bugs
        ]
        cur.executemany(
    ...
    def get_bugs(self) -> list[Bug]:
        cur = self.cursor()
        cur.execute(
            "SELECT id, cve, reference, reference_url, bug_type, severity, category, component, updated_versions, bulletin FROM advisories ORDER BY component"
        )
        rows = cur.fetchall()
...
def get_bugs_from_table(
    table: PageElement, bulletin: str, category: BugCategory, is_kernel: bool = False
) -> list[Bug]:
    ...
    for row in rows:
        ...
        updated_versions = [version.strip(",") for version in updated_versions]
        ...
        if is_kernel:
            ...
            component = (
                "" if reference_url is None else classify_bug_component(reference_url)
            )
            updated_versions = cols[4].get_text(strip=True).split()
            updated_versions = [version.strip(",") for version in updated_versions]
...
def import_advisories(db: Db, bulletin: str):
    ...
    year, _ = bulletin.split("-")
    if int(year) >= 2026:
        url = f"https://source.android.com/docs/security/bulletin/{year}/{bulletin}-01"
    else:
    ...
    for category in BugCategory:
        ...
        if table is not None:
            advisories.extend(
                get_bugs_from_table(
                    table, bulletin, category, is_kernel=category.is_kernel()
                )
            )
...
def reclassify_bugs(db: Db):
    ...
    for bug in bugs:
        if not bug.category.is_kernel():
            bug.component = (
                ""
                if bug.reference_url is None
                else classify_bug_component(bug.reference_url)
            )
...
def list_advisories(db: Db, args: argparse.Namespace):
    ...
    month = None
    if args.after_date:
        a, b = args.after_date.split("-")
        year = int(a)
        month = int(b)
        if not (0 < month <= 12):
            print("invalid --after-date month")
            return
    ...
    for bug in db.get_bugs():
        ...
        if (
            ...
            and not bug.is_after_security_bulletin(year, month)
        ):
            continue
...
def main():
    ...
    import_parser = subparsers.add_parser(
        "import", help="Import advisories from a given URL"
    )
    import_parser.add_argument("date", help="Date of android advisory (yyyy-mm format)")
    ...
    list_parser.add_argument(
        "--category", type=str, help="Filter based on bug category"
    )
    list_parser.add_argument(
        "--component", type=str, help="Filter based on bug affected component"
    )
    list_parser.add_argument("--type", type=str, help="Filter based on bug type")
    list_parser.add_argument(
        "--severity", type=str, help="Filter based on bug severity"
    )
    list_parser.add_argument(
        "--after-date",
        type=str,
        help="Filter based on patches after a given date (yyyy-mm format)",
    )
    ...
    if args.command == "import":
        import_advisories(db, args.date)
