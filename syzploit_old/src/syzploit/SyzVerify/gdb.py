...
_kaslr_offset = None  # KASLR slide: runtime - static (set via config or auto-detected)
...
def _resolve_symbol(name):
    ...
    3. System.map symbols (with KASLR offset applied if available)
    """
    ...
    # Fall back to System.map (apply KASLR offset if known)
    static_addr = _system_map.get(name)
    if static_addr is not None and _kaslr_offset is not None:
        return static_addr + _kaslr_offset
    return static_addr


def _compute_kaslr_offset():
    """
    Auto-detect KASLR slide by comparing a runtime address against the
    static System.map.  Requires both _system_map and at least one
    runtime symbol to be already loaded.

    Sets the global _kaslr_offset on success.
    """
    global _kaslr_offset

    if not _system_map:
        return

    # Collect all known runtime addresses (alloc + free + crash stack)
    runtime = {}
    runtime.update(_runtime_alloc_addrs)
    runtime.update(_runtime_free_addrs)
    runtime.update(_crash_stack_addrs)

    if not runtime:
        return

    # Find a symbol present in both maps
    ref_symbols = ["_text", "_stext", "__kmalloc", "kfree", "schedule",
                   "kmem_cache_alloc", "kmem_cache_free"]
    for sym in ref_symbols:
        if sym in runtime and sym in _system_map:
            offset = runtime[sym] - _system_map[sym]
            # Sanity: should be page-aligned
            if offset & 0xFFF:
                _log(f"KASLR: offset 0x{offset:x} via {sym} is not page-aligned, skipping")
                continue
            _kaslr_offset = offset
            _log(f"KASLR offset auto-detected: 0x{offset:x} (via {sym})")

            # Cross-validate with another symbol
            for v in ref_symbols:
                if v != sym and v in runtime and v in _system_map:
                    expected = _system_map[v] + offset
                    if expected == runtime[v]:
                        _log(f"  Validated via {v}: OK")
                    else:
                        _log(f"  WARNING: {v} mismatch: expected 0x{expected:x}, "
                             f"got 0x{runtime[v]:x}")
                    break
            return

    _log("KASLR: no common reference symbol found between runtime and System.map")
...
def _install_breakpoint_with_fallback(addr, name, bp_class=None, prefer_hw=True, allow_sw_fallback=True):
    """Install a breakpoint at *addr*, trying both HW and SW mechanisms.

    Strategy:
      - If prefer_hw: try HW first, then SW.
      - Else: try SW first, then HW (useful when SW "cannot access memory"
        during late-boot/paravirt but HW debug registers still work).

    Args:
        addr: Address to install breakpoint
        name: Symbol name for logging
        bp_class: Optional breakpoint class to use for SW breakpoints
        prefer_hw: Try hardware breakpoint first
        allow_sw_fallback: Allow falling back to SW when HW fails
                           (set False during early boot to avoid unmapped writes)
    """
    if prefer_hw and not _hw_not_supported:
        if _install_hw_breakpoint(addr, name):
            return True
        # HW failed — try SW if allowed
        if allow_sw_fallback:
            _log(f"HW failed for {name}, falling back to SW")
            return _install_sw_breakpoint(addr, name, bp_class)
        return False

    # Prefer SW
    if allow_sw_fallback:
        ok = _install_sw_breakpoint(addr, name, bp_class)
        if ok:
            return True
        # SW failed — try HW as last resort (e.g. memory not yet mapped
        # for SW patching but HW debug registers still work)
        if not _hw_not_supported:
            _log(f"SW failed for {name}, trying HW fallback")
            return _install_hw_breakpoint(addr, name)
        return False

    # Both disabled
    _log(f"No breakpoint mechanism available for {name}")
    return False
...
def install_alloc_breakpoints():
    ...
    def _install_bp(addr, name, is_alloc=True):
        """Install breakpoint using the robust fallback mechanism."""
        bp_class = AllocBreakpoint if is_alloc else FreeBreakpoint
        return _install_breakpoint_with_fallback(
            addr, name, bp_class=bp_class,
            prefer_hw=not use_sw,  # prefer HW during early boot
            allow_sw_fallback=use_sw,
        )
    ...
    if _runtime_symbols_loaded:
        ...
        for name, addr in _runtime_alloc_addrs.items():
            ...
            if name in _hot_skip:
                ...
                continue
            if _install_bp(addr, name, is_alloc=True):
                _installed_symbols.add(name)
        ...
        for name, addr in _runtime_free_addrs.items():
            ...
            if not use_sw and installed >= _max_hw_bps:
                break  # HW breakpoint limit
            if _install_bp(addr, name, is_alloc=False):
                _installed_symbols.add(name)
        ...
        for var, name, is_alloc in functions:
            ...
            if addr:
                if _install_bp(addr, name, is_alloc=is_alloc):
                    _installed_symbols.add(name)
                    if is_alloc:
                        _alloc_addrs.add(addr)
                    else:
...
def install_crash_stack_breakpoints():
    """Install breakpoints on crash stack functions (SW preferred, HW fallback)."""
    installed = 0
    ...
    for func_name, addr in _crash_stack_addrs.items():
        if func_name in _installed_symbols:
            ...
            continue
        if _install_breakpoint_with_fallback(addr, func_name, bp_class=FuncBreakpoint,
                                             prefer_hw=False, allow_sw_fallback=True):
    ...
    try:
        ...
        results = {
            ...
            "tracing_info": {
                ...
                "kaslr_offset": hex(_kaslr_offset) if _kaslr_offset is not None else None,
                "demo_mode": is_demo,
...
def _load_config_json(path):
    ...
    try:
        ...
        # Load explicit KASLR offset from config, or auto-detect
        global _kaslr_offset
        if cfg.get("kaslr_offset") is not None:
            try:
                val = cfg["kaslr_offset"]
                _kaslr_offset = int(val, 16) if isinstance(val, str) else int(val)
                _log(f"  KASLR offset from config: 0x{_kaslr_offset:x}")
            except Exception as e:
                _log(f"  Failed to parse kaslr_offset: {e}")

        if _kaslr_offset is None:
            _compute_kaslr_offset()
