...
from ..utils.adb import calculate_adb_port, get_adb_target
from ..utils.compilation import compile_exploit
from .cuttlefish import CuttlefishController, CuttlefishConfig, CuttlefishKernelGDB
...
# Use set_adb_target() or pass instance= to functions to configure
ADB_TARGET_DEVICE: str | None = None


def set_adb_target(instance: Optional[int] = None, target: Optional[str] = None) -> None:
    """
    Set the ADB target device for subsequent operations.
    
    Args:
        instance: Cuttlefish instance number (1-based). Takes precedence if both provided.
        target: Explicit ADB target string (e.g., '0.0.0.0:6524')
        
    Example:
        >>> set_adb_target(instance=5)  # Sets to '0.0.0.0:6524'
        >>> set_adb_target(target='localhost:6520')
    """
    global ADB_TARGET_DEVICE
    if instance is not None:
        ADB_TARGET_DEVICE = get_adb_target(instance)
    elif target is not None:
        ADB_TARGET_DEVICE = target
    else:
...
def adb_upload_file(file: Path, upload_location: Path, retries: int = 3, retry_delay: int = 5):
    file = Path(file).resolve()
    if not file.exists():
        raise FileNotFoundError(
            f"Cannot push to device: local file does not exist: {file}"
        )
    cmd = _adb_cmd('push', str(file), str(upload_location))
    ...
    for attempt in range(1, retries + 1):
        ...
        # Print the ADB error prominently so it's always visible in logs,
        # since CalledProcessError.__str__() does NOT include stderr.
        print(f"[ERROR] adb push failed (exit {result.returncode}, attempt {attempt}/{retries}):",
              flush=True)
        ...
        last_err = subprocess.CalledProcessError(
            ...
            stderr=f"adb push failed: {stderr_msg}",
        )

        if attempt < retries:
            print(f"[RETRY] Waiting {retry_delay}s before retry...", flush=True)
...
def _verify_crash_path(dynamic_results: dict, parsed_crash: dict) -> dict:
    ...
    if not dynamic_results.get("crash_detected", False):
        reasons = []
        # Determine the vulnerability kind for accurate messaging
        vuln_kind = (parsed_crash.get("kind") or "").lower()
        if "use-after-free" in vuln_kind:
            vuln_label = "use-after-free"
            silent_desc = "the stale pointer dereference accesses recycled/valid memory"
        elif "out-of-bounds" in vuln_kind or "slab-out-of-bounds" in vuln_kind:
            vuln_label = "out-of-bounds access"
            silent_desc = "the overrun reads/writes adjacent slab memory without hitting a red-zone"
        elif "null-ptr-deref" in vuln_kind or "null pointer" in vuln_kind:
            vuln_label = "null-pointer dereference"
            silent_desc = "the null deref may be caught by userfaultfd or mapped page 0"
        elif "double-free" in vuln_kind or "double free" in vuln_kind:
            vuln_label = "double-free"
            silent_desc = "the second free corrupts slab metadata silently"
        elif "race" in vuln_kind or "data-race" in vuln_kind:
            vuln_label = "data race"
            silent_desc = "the race condition may not manifest without specific timing"
        elif "stack-out-of-bounds" in vuln_kind:
            vuln_label = "stack buffer overflow"
            silent_desc = "the stack overrun corrupts adjacent stack variables silently"
        elif "uninit" in vuln_kind or "kmsan" in vuln_kind:
            vuln_label = "uninitialized memory use"
            silent_desc = "the uninitialized read returns stale stack/heap data without faulting"
        else:
            vuln_label = vuln_kind.split(":", 1)[-1].strip() if vuln_kind else "memory safety violation"
            silent_desc = "the memory corruption proceeds silently without sanitiser detection"

        # Check if KASAN is enabled
        kernel_ver = dynamic_results.get("kernel_version", "")
        if "kasan" not in kernel_ver.lower():
            reasons.append(
                f"KASAN not enabled — {vuln_label} proceeds silently "
                f"instead of triggering a panic"
            )
        # Check offsets  
        if best_chain_depth >= 2 and score >= 0.4:
            reasons.append(
                f"Code path was exercised but without KASAN "
                f"{silent_desc} (different offsets "
                f"or timing) so no fault is raised"
            )
        if reasons:
...
def test_repro_with_cuttlefish_controller(
    ...
    try:
        ...
        # Pre-flight: verify device is connected and report ADB state
        print(f"[PREFLIGHT] repro_path = {repro_path}")
        print(f"[PREFLIGHT] repro exists = {Path(repro_path).exists()}")
        print(f"[PREFLIGHT] ADB_TARGET_DEVICE = {ADB_TARGET_DEVICE}")
        print(f"[PREFLIGHT] ADB_EXE_PATH = {ADB_EXE_PATH}")
        try:
            dev_check = subprocess.run(
                _adb_cmd('devices'), capture_output=True, text=True, timeout=10
            )
            print(f"[PREFLIGHT] adb devices:\n{dev_check.stdout.strip()}")
        except Exception as e:
            print(f"[PREFLIGHT] adb devices failed: {e}")
...
def verify_exploit_privilege_escalation(
    ...
    if instance is not None:
        adb_port = calculate_adb_port(instance)
...
# compile_exploit is imported from ..utils.compilation
