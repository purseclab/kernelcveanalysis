...
def fix_with_llm(c_code: str, errors: List[CompilationError], target_arch: str) -> Optional[str]:
    ...
    # Get API key from environment (supports OpenAI, OpenRouter, Anthropic, Gemini)
    api_key = (get_api_key() if get_api_key else None) or ""
    if not api_key:
        print("[WARN] No LLM API key found in environment (OPENAI_API_KEY / OPENROUTER_API_KEY / etc.), cannot use LLM for fixes")
        return None
