...
class RuntimeSymbols:
    ...
    kaslr_offset: Optional[int] = None  # computed KASLR slide
    static_map: Optional[Dict[str, int]] = None  # static System.map symbols
    ...
    def compute_kaslr_offset(
        self,
        static_map_path: Optional[str] = None,
        reference_symbol: str = "_text",
        logger: Optional[Callable] = None,
    ) -> Optional[int]:
        """
        Compute the KASLR slide by comparing a reference symbol's runtime
        address (from kallsyms) against its static address (from System.map).

        KASLR offset = runtime_address - static_address

        Once computed, the offset is cached in self.kaslr_offset and the
        static map is stored in self.static_map for later use.

        Args:
            static_map_path: Path to the *static* System.map (compiled into
                the kernel image). If None, tries common locations.
            reference_symbol: Symbol to compare (default: "_text").
            logger: Optional logging callback.

        Returns:
            The signed KASLR offset in bytes, or None on failure.
        """
        def log(msg):
            if logger:
                logger(msg)
        ...
        # Load static System.map if not already loaded
        if self.static_map is None:
            if static_map_path and os.path.isfile(static_map_path):
                self.static_map = _load_static_system_map(static_map_path)
                log(f"Loaded static System.map: {len(self.static_map)} symbols")
            else:
                log(f"Static System.map not found: {static_map_path}")
                return None

        if not self.static_map:
            log("Static System.map is empty")
            return None

        # Get runtime address of reference symbol
        runtime_addr = self.get_address(reference_symbol)
        if runtime_addr is None:
            # Try fallback reference symbols
            fallbacks = ["_stext", "_text", "startup_64", "primary_entry",
                         "__start_rodata", "_etext"]
            for sym in fallbacks:
                runtime_addr = self.get_address(sym)
                if runtime_addr is not None:
                    reference_symbol = sym
                    break
            if runtime_addr is None:
                log(f"Reference symbol '{reference_symbol}' not found in runtime kallsyms")
                return None

        # Get static address
        static_addr = self.static_map.get(reference_symbol)
        if static_addr is None:
            log(f"Reference symbol '{reference_symbol}' not found in static System.map")
            return None

        offset = runtime_addr - static_addr

        # Sanity check: offset should be page-aligned (4K)
        if offset & 0xFFF:
            log(f"WARNING: KASLR offset 0x{offset:x} is not page-aligned — may be incorrect")

        self.kaslr_offset = offset
        log(f"KASLR offset: 0x{offset:x}  (runtime 0x{runtime_addr:x} - static 0x{static_addr:x})")
        log(f"  Reference symbol: {reference_symbol}")

        # Cross-validate with a second symbol if available
        validators = ["kfree", "__kmalloc", "schedule", "do_sys_open"]
        for v in validators:
            rt = self.get_address(v)
            st = self.static_map.get(v)
            if rt is not None and st is not None:
                expected = st + offset
                if expected == rt:
                    log(f"  Validated via {v}: OK")
                else:
                    diff = rt - expected
                    log(f"  WARNING: {v} mismatch: expected 0x{expected:x}, "
                        f"got 0x{rt:x} (diff={diff:+d})")
                break

        return offset

    def resolve_static_symbol(self, name: str) -> Optional[int]:
        """
        Resolve a symbol from the static System.map, applying the KASLR
        offset to return the actual runtime address.

        Useful for symbols present in System.map but missing from kallsyms
        (e.g. static/inline functions that don't export).

        Returns:
            Runtime address (static + kaslr_offset), or None.
        """
        if self.static_map is None or self.kaslr_offset is None:
            return None
        static_addr = self.static_map.get(name)
        if static_addr is None:
            return None
        return static_addr + self.kaslr_offset

    def resolve_address(self, name: str) -> Optional[int]:
        """
        Best-effort symbol resolution: check runtime kallsyms first, then
        fall back to static System.map + KASLR offset.

        This is the preferred lookup to use downstream.
        """
        addr = self.get_address(name)
        if addr is not None:
            return addr
        return self.resolve_static_symbol(name)

    def merge_with_static_map(
        self,
        static_map_path: str,
        logger: Optional[Callable] = None,
    ) -> int:
        """
        Merge symbols from a static System.map into the runtime symbol
        table, applying the KASLR offset.  Only fills in symbols that are
        *not* already present in the runtime set.

        Args:
            static_map_path: Path to the static System.map.
            logger: Optional logging callback.

        Returns:
            Number of symbols added from the static map.
        """
        def log(msg):
            if logger:
                logger(msg)
        ...
        # Ensure KASLR offset is computed
        if self.kaslr_offset is None:
            self.compute_kaslr_offset(static_map_path, logger=logger)

        if self.kaslr_offset is None:
            log("Cannot merge: KASLR offset unknown")
            return 0
        ...
        added = 0
        for name, static_addr in self.static_map.items():
            if name not in self.symbols:
                runtime_addr = static_addr + self.kaslr_offset
                self.symbols[name] = SymbolInfo(
                    address=runtime_addr,
                    symbol_type="T",  # assume text; static map has real types
                    name=name,
                    module=None,
                )
                added += 1

        log(f"Merged {added} symbols from static System.map (kaslr_offset=0x{self.kaslr_offset:x})")
        return added

    def translate_address(self, static_addr: int) -> Optional[int]:
        """Translate a raw static address to runtime using the KASLR slide."""
        if self.kaslr_offset is None:
            return None
        return static_addr + self.kaslr_offset

    def reverse_translate(self, runtime_addr: int) -> Optional[int]:
        """Convert a runtime address back to its static System.map address."""
        if self.kaslr_offset is None:
            return None
        return runtime_addr - self.kaslr_offset


def _load_static_system_map(path: str) -> Dict[str, int]:
    """
    Load a static System.map file into a name -> address dict.

    This is the System.map produced at kernel compile time (before KASLR).
    """
        ...
        for line in f:
            ...
            if len(parts) >= 3:
                try:
                    addr = int(parts[0], 16)
                    name = parts[2]
                    if name not in symbols:  # keep first occurrence
                        symbols[name] = addr
                except ValueError:
                    continue
    return symbols
