...
def analyze_crash_to_postcondition(parsed: Dict[str, Any], snippets: Dict[str, Any], evidence: Dict[str, Any]) -> PostCondition:
    ...
    return pc
"""
LLM adapter: routes chat completions through LiteLLM.

LiteLLM auto-detects the provider from the model string prefix and uses
the matching API-key environment variable.  Supported providers include:

  Model prefix                     Env var needed
  ────────────────────────────────────────────────
...
Set your key in the shell environment or in a ``.env`` file (see
``utils/env.py`` for lookup locations).

Usage example (OpenRouter):
    export OPENROUTER_API_KEY=sk-or-v1-...
    syzploit generate-exploit ./analysis --model openrouter/anthropic/claude-sonnet-4-20250514
"""
def llm_chat(model: str, messages: list):
    """Unified chat completion via LiteLLM.

    LiteLLM auto-detects the provider from the *model* string and reads
    the matching API-key env-var automatically:

    - ``gpt-4o``                     → needs ``OPENAI_API_KEY``
    - ``openrouter/anthropic/...``   → needs ``OPENROUTER_API_KEY``
    - ``anthropic/claude-...``       → needs ``ANTHROPIC_API_KEY``
    - ``gemini/...``                 → needs ``GEMINI_API_KEY``

    Set the key either in the shell environment or in your ``.env`` file
    (loaded by ``utils.env.load_env``).

    Args:
        model: LiteLLM model identifier (e.g. ``openrouter/google/gemini-2.5-pro``).
        messages: List of ``{role, content}`` dicts.

    Returns:
        LiteLLM ``ModelResponse`` (OpenAI-compatible).
    """
    # Ensure .env is loaded so API keys are visible to litellm
