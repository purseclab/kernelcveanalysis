...
@dataclass
class _LegacyExploitPlan:
    """Legacy plan representation — kept for backward compatibility.
    New code should use ExploitPlan from core.py."""
    steps: List[str]
...
EXPLOITATION_PATTERNS = {
    ...
    "tty_struct_uaf": {
        "description": "UAF exploited via tty_struct spray and ops hijack",
    ...
    "seq_operations_uaf": {
        "description": "UAF exploited via seq_operations spray and function pointer hijack",
        "target_struct": "seq_operations",
        "slab_cache": "kmalloc-64",
        "technique": "seq_ops_hijack",
        "steps": [
            "trigger_uaf",
            "spray_seq_operations",
            "corrupt_seq_ops_pointer",
            "trigger_read_for_rip_control",
            "stack_pivot",
            "rop_to_commit_creds",
        ],
        "code_hints": {},
    ...
    "sk_buff_uaf": {
        "description": "UAF exploited via sk_buff spray for info leak or corruption",
        "target_struct": "sk_buff",
        "slab_cache": "kmalloc-*",
        "technique": "sk_buff_corruption",
        "steps": [
            "trigger_uaf",
            "spray_sk_buff",
            "corrupt_skb_data",
    ...
    "cross_cache_uaf": {
        "description": "UAF exploited via cross-cache attack (page-level reclaim)",
        "target_struct": "unknown",
        "slab_cache": "dedicated",
        ...
        "steps": [
            ...
            "derive_arb_read_write",
            "escalate_privileges",
        ],
        "code_hints": {},
    },
    "dirty_pagetable": {
        "description": "Page-level UAF exploited via dirty pagetable technique",
        "target_struct": "pte",
        "slab_cache": "page_alloc",
        "technique": "dirty_pagetable",
        "steps": [
            "trigger_uaf",
            "free_page",
            "allocate_page_tables_on_freed_page",
            "corrupt_pte",
            "map_physmem_via_pte",
            "overwrite_cred_via_physmap",
            "escalate_privileges",
        ],
        "code_hints": {},
    },
    "oob_write_generic": {
        "description": "Out-of-bounds write exploitation via heap manipulation",
        "target_struct": "unknown",
        "slab_cache": "kmalloc-*",
        "technique": "adjacent_object_corruption",
        "steps": [
            "trigger_oob_write",
            "spray_target_objects",
            "corrupt_adjacent_object",
            "derive_capability_from_corruption",
            "bypass_kaslr",
            "escalate_privileges",
        ],
        "code_hints": {},
    ...
    "oob_read_info_leak": {
        "description": "Out-of-bounds read for information leak leading to priv-esc",
        "target_struct": "unknown",
        "slab_cache": "kmalloc-*",
        "technique": "info_leak_to_arb_write",
        "steps": [
            "trigger_oob_read",
            "leak_kernel_pointers",
            "bypass_kaslr",
            "chain_with_second_vuln",
            "escalate_privileges",
        ],
        "code_hints": {},
    },
    "race_condition_generic": {
        "description": "Race condition exploitation via TOCTOU or data race",
        "target_struct": "unknown",
        "slab_cache": "kmalloc-*",
        "technique": "race_to_uaf_or_corruption",
        "steps": [
            "trigger_race_condition",
            "exploit_race_outcome",
            "spray_to_reclaim",
            "derive_capability",
            "escalate_privileges",
        ],
    ...
    "double_free_generic": {
        "description": "Double-free exploitation via heap spray",
        "target_struct": "unknown",
        "slab_cache": "kmalloc-*",
        "technique": "double_free_reclaim",
        "steps": [
            "trigger_double_free",
            "spray_to_reclaim_first_slot",
            "trigger_second_free",
            "spray_controlled_objects",
            "corrupt_overlapping_object",
            "escalate_privileges",
        ],
        "code_hints": {},
    },
    "modprobe_path_overwrite": {
        "description": "Arbitrary write to modprobe_path for privilege escalation",
        "target_struct": "modprobe_path",
        "slab_cache": "n/a",
        "technique": "modprobe_path_hijack",
        "steps": [
            "trigger_vulnerability",
            "obtain_arb_write",
            "bypass_kaslr",
            "overwrite_modprobe_path",
            "trigger_modprobe_exec",
        ],
        "code_hints": {},
    },
}
...
Output ONLY the JSON, no other text."""
    ...
    try:
        ...
        response = get_openai_response(prompt, api_key, model=model)
        if response:
...
def generate_exploit_plan(
    ...
    if use_llm_refinement:
        plan = generate_plan_with_llm(analysis_data, pattern, target_arch, model=model)
    else:
